...
def solve_discrete_SIHR(init, beta, gamma, epsilon, N, timesteps, delta_t=0.001):
    """
        Discrete version of SIR model.
    ...
    num_repetitions = np.int(np.ceil(1/delta_t))
    num_points = num_repetitions*timesteps #1 day will have 1/delta_t timesteps
    S, I, H, R = np.zeros(num_points), np.zeros(num_points), np.zeros(num_points), np.zeros(num_points)

    S[0], I[0], H[0],R[0] = S0, I0, H0, R0
    ...
    for t in range(num_points-1):
        ...
        I[t+1] = I[t] + delta_t*(rate_s2i - rate_i2h)
